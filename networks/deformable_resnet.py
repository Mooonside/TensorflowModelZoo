...
from tf_ops.cuda_ops import deform_conv2d


def resnet_arg_scope_deform_gn(weight_decay=0.0001, group_norm_num=32, group_norm_dim=None):
    ...
    with arg_scope(
        ...
        with arg_scope([deform_conv2d],
                       weights_regularizer=regularizers.l2_regularizer(weight_decay),
                       weights_initializer=zeros_initializer,
                       # weights_initializer=initializers.variance_scaling_initializer(),
                       activation_fn=None,
                       normalizer_fn=group_norm2d
                       ):
...
def deformable_bottleneck(inputs,
    ...
    with variable_scope.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
        ...
        if deformable:
            residual = deform_conv2d(residual,
                                     depth_bottleneck,
                                     3,
                                     stride=stride,
                                     rate=rate,
...
def resnet_v1_block(scope, base_depth, num_units,  stride, deformable_units=0, rate=1):
    ...
    return resnet_utils.Block(scope, deformable_bottleneck,
                              [{
                                  'depth': base_depth * 4,
...
def resnet_v1_50_deform(inputs,
                 num_classes=None,
                 is_training=True,
                 global_pool=True,
                 reuse=None,
    ...
    blocks = [
        resnet_v1_block('block1', base_depth=64,  num_units=3, deformable_units=0, stride=1, rate=rates[0]),
        resnet_v1_block('block2', base_depth=128, num_units=4, deformable_units=0, stride=2, rate=rates[1]),
        resnet_v1_block('block3', base_depth=256, num_units=6, deformable_units=0, stride=2, rate=rates[2]),
        resnet_v1_block('block4', base_depth=512, num_units=3, deformable_units=2, stride=2, rate=rates[3]),
    ]
    return resnet_v1(
        inputs,
        blocks,
...
def resnet_v1_101_deform(inputs,
                  num_classes=None,
                  is_training=True,
                  global_pool=True,
                  reuse=None,
    ...
    blocks = [
        resnet_v1_block('block1', base_depth=64,  num_units=3, deformable_units=0, stride=1, rate=rates[0]),
        resnet_v1_block('block2', base_depth=128, num_units=4, deformable_units=0, stride=2, rate=rates[1]),
        resnet_v1_block('block3', base_depth=256, num_units=23, deformable_units=0,stride=2, rate=rates[2]),
        resnet_v1_block('block4', base_depth=512, num_units=3, deformable_units=2, stride=2, rate=rates[3]),
    ]
    return resnet_v1(
        inputs,
...
def resnet_v1(inputs,
    ...
    with variable_scope.variable_scope(
        ...
        with arg_scope(
                [conv2d, deformable_bottleneck, stack_blocks_dense, max_pool2d],
...
if __name__ == '__main__':
    ...
    with tf.device('/CPU:0'):
        ...
        with arg_scope(resnet_arg_scope_deform_gn(weight_decay=1e-4, group_norm_num=32)):
            net, end_points = resnet_v1_101_deform(
                inputs,
                num_classes=1001,
                is_training=True,
                global_pool=True,
