...
from datasets.ilsvrc.ilsvrc_reader import TRAIN_DIR, TRAIN_NUM, VALID_NUM, VALIDATION_DIR,\
    get_dataset, get_next_batch, inception_augmentation
from networks.deformable_resnet import resnet_v1_50_deform, resnet_arg_scope_deform_gn
...
flags.DEFINE_string('pretrained_ckpts', '/home/chenyifeng/TF_Models/ptrain/ILSVRC/ResNet_50_V1_GN/model.ckpt',
                    'where to load pretrained model')

flags.DEFINE_string('last_ckpt', None,
                    'where to load last saved model')

flags.DEFINE_string('next_ckpt', '/home/chenyifeng/TF_Models/atrain/TensorflowModelZoo/resnet50_deform/',
...
def inference_to_loss(image_batch, label_batch, is_training=True):
    with arg_scope(resnet_arg_scope_deform_gn(weight_decay=FLAGS.weight_decay, group_norm_num=32)):
        ...
        net, end_points = resnet_v1_50_deform(image_batch,
